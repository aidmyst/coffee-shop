    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 py-8" x-data="{
        ...
        <div
            ...
            <div class="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
                ...
                {{-- TOMBOL TAMBAH --}}
                <a href="{{ route('admin.inventory.add_stock') }}"
        ...
        {{-- Statistik Dinamis --}} <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            <div class="bg-white p-4 rounded-xl shadow-sm border-l-4 border-blue-500">
                <p class="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Total Bahan</p>
                <h3 class="text-2xl font-black text-blue-800 mt-1">{{ $totalItems }} <span
            ...
            <div class="bg-white p-4 rounded-xl shadow-sm border-l-4 border-green-500">
                ...
                <h3 class="text-2xl font-black text-green-600 mt-1">{{ $safeStock }} <span
                        class="text-sm text-gray-400 font-normal">Item</span></h3>
            </div>
            <div class="bg-white p-4 rounded-xl shadow-sm border-l-4 border-orange-500">
                ...
                <h3 class="text-2xl font-black text-orange-600 mt-1">{{ $lowStock }} <span
                        class="text-sm text-gray-400 font-normal">Item</span></h3>
            </div>
            <div class="bg-white p-4 rounded-xl shadow-sm border-l-4 border-red-500">
                <p class="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Stok Habis</p>
                <h3 class="text-2xl font-black text-red-600 mt-1">{{ $outOfStock }} <span
                        class="text-sm text-gray-400 font-normal">Item</span></h3>
            </div>
        </div>
                    ...
                    @foreach ($ingredients as $item)
                        <tr class="hover:bg-gray-50 transition-colors"
                            ...
                            <td class="px-6 py-4 text-center">
                                <span
                                    class="px-3 py-1.5 rounded-lg text-[10px] font-black uppercase {{ $item->stock <= $item->min_stock ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700' }}">
                            ...
                            <td class="px-6 py-4 text-center">
                                ...
                                <button
                                    ...
                                    editData = { 
                                        ...
                                        {{-- Gunakan Math.round untuk menghilangkan desimal saat masuk ke modal --}}
                                        stock: Math.round('{{ $item->stock }}'), 
                                        min_stock: Math.round('{{ $item->min_stock }}'), 
                                        unit: '{{ $item->unit }}' 
                                    }; 
                                    showEditModal = true"
