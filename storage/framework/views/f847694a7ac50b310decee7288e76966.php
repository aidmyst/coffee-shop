...
<?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::processComponentKey($component); ?>
    ...
    x-init="$watch('customerName', val => localStorage.setItem('cafe_customer_name', val))">
        ...
        <div x-show="showCartModal" x-cloak x-transition:enter="transition duration-300 transform"
            ...
            <div class="overflow-y-auto flex-1 px-6 py-4 custom-scrollbar">
                ...
                <div class="mt-4">
                    ...
                    <input type="text" x-model="customerName" placeholder="Your Name"
                        class="w-full bg-white border border-gray-100 rounded-2xl text-sm p-4 focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 text-gray-700 shadow-sm outline-none" required />
                </div>
                ...
                <div class="mt-4">
                    ...
                    <textarea x-model="orderNote" placeholder="Example: Add spoon or fork please..."
                        class="w-full bg-white border border-gray-100 rounded-2xl text-sm p-4 focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 min-h-[80px] resize-none text-gray-700 shadow-sm outline-none"></textarea>
                </div>
            </div>
    ...
    <script>
        function cartSystem() {
            return {
                ...
                onTouchEnd() {
                    this.dragging = false;
                    this.$refs.modal.style.transition = 'transform 0.3s cubic-bezier(0.4, 0, 0.2, 1)';
                ...
                async submitOrder() {
                    ...
                    if (!this.customerName || this.customerName.trim() === '') {
                        alert('Silakan isi nama Anda terlebih dahulu sebelum Checkout.');
                        return;
                    }

                    const tokenTag = document.querySelector('meta[name="csrf-token"]');
                    if (!confirm('Lanjutkan ke pembayaran untuk pesanan Meja ' + this.tableNumber + '?')) return;
                        ...
                        if (response.ok) {
                            ...
                            if (result.success && result.snap_token) {
                                ...
                                window.snap.pay(result.snap_token, {
                                    ...
                                    onClose: function () {
                                        alert('Form pembayaran ditutup. Pesanan tetap tercatat, silakan bayar di kasir jika tidak jadi via online.');
                                        component.cart = [];
                                        component.orderNote = '';
                                        localStorage.clear();
                                        component.showCartModal = false;
                                ...
                                alert('Pesanan Berhasil dikirim! Silakan bayar di kasir.');
                                this.cart = [];
                                this.orderNote = '';
                                localStorage.clear();
